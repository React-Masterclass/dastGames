...
export default function useGamesDeveloper() {
  ...
  useEffect(() => {
    async function getAPI() {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}games?key=${import.meta.env.VITE_API_KEY}&page=${pagination}&page_size=20&developers=${developer}&search=${search}`
        );
        if (response.ok) {
    ...
    getAPI();
  }, [pagination, debouncedSearch , developer]);
  ...
  const setDeveloperAndFetch = (newDeveloper) =>{
  ...
  return {
    ...
    pagination,
    search,
    setDeveloper : setDeveloperAndFetch,
