...
export default function useGamesGenre() {
  ...
  useEffect(() => {
    async function getAPI() {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}games?key=${import.meta.env.VITE_API_KEY}&page=${pagination}&page_size=20&genres=${genre}&search=${search}`
        );
        if (response.ok) {
    ...
    getAPI();
  }, [pagination, debouncedSearch, genre]);
  ...
  const setGenreAndFetch = (newGenre) =>{
    setGenre(newGenre);
  };
